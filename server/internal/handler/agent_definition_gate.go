...
// Agent-definition write gate (Jartan fork, SEC-2026-0069).
//
...
// the agent-record mutation surface, with one addition Vale ruled and Thorne
// scoped: an owner-configured allow-list, so People Ops' standing authority to
...
//
// The governing text is Thorne's amendment on JAR-571, comment 0b3ad55b
// (2026-08-18), which supersedes the original ruling 83bcdfb0 wherever the two
// differ. Section references in this file are to that amendment.
//
...
// ruled model (JAR-575) constrains agent principals; a cloud node registering
...
// is evidence, not a control: Thorne's JAR-521 ruling stands that an audit
// trail alone does not close SEC-2026-0069 (write, act, write back and the
// trail shows two edits with a zero diff). It exists so abuse of the one live
...
//
// Removed from the pre-amendment list, each with Thorne's live evidence:
...
// `private`: unassignable, needing the owner to flip a field per hire (the Ward
// precedent, JAR-498).
//
