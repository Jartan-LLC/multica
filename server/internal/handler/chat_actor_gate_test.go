...
// Tests for chat-session access by principal (Jartan fork).
//
// Chat authorisation keyed on `chat_session.creator_id` alone, and the auth
// middleware stamps a task token's `X-User-ID` with the agent's owning human,
// so every seat and the owner satisfy the same creator predicate.
//
// What the assertions below require: an ordinary agent seat's session list
// contains only sessions belonging to that seat, and a request against an
// owner-created session returns 403 or 404.
//
...
// The closure condition: a session list from an
...
func TestChatActorGate_AgentSeatListsOnlyItsOwnSessions(t *testing.T) {
	...
	t.Run("an agent seat sees only its own", func(t *testing.T) {
		...
		if ids[ownerWithNoema] {
			t.Fatalf("Ferro's run listed the owner's private session with another agent (%s) — the principal scope is not applied", ownerWithNoema)
		}
		if !ids[ferroToAnvil] {
