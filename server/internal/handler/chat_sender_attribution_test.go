...
// Tests for agent-sent chat attribution (Jartan fork, SEC-2026-0079).
//
...
// assertions below are Thorne's closure condition on JAR-616: a message sent
...
func TestChatSenderAttribution_AgentSendRecordsTheSendingAgent(t *testing.T) {
	...
	if row.OriginatorSource == nil || *row.OriginatorSource == "direct_human" {
		t.Fatalf("originator_source = %v, want anything but direct_human — SEC-2026-0079's false claim is intact", row.OriginatorSource)
	}
