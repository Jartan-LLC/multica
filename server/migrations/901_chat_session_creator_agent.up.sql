...
COMMENT ON COLUMN chat_session.creator_agent_id IS
    'The agent whose run created this session, or NULL when a human created it (Jartan fork). Read by the chat actor gate: a task-token request reaches a session only when the calling agent is the session''s agent_id or its creator_agent_id. Not a foreign key — the relationship is maintained in the application layer.';
